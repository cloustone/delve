...
func ARM64Arch(goos string) *Arch {
	return &Arch{
		...
		RegistersToDwarfRegisters:        mips64RegistersToDwarfRegisters,
		addrAndStackRegsToDwarfRegisters: mips64AddrAndStackRegsToDwarfRegisters,
		DwarfRegisterToString:            mips64DwarfRegisterToString,
		inhibitStepInto:                  func(*BinaryInfo, uint64) bool { return false },
		asmDecode:                        mips64AsmDecode,
	}
}

func mips64FixFrameUnwindContext(fctxt *frame.FrameContext, pc uint64, bi *BinaryInfo) *frame.FrameContext {
	a := bi.Arch
	...
	if fctxt == nil || (a.sigreturnfn != nil && pc >= a.sigreturnfn.Entry && pc < a.sigreturnfn.End) {
		...
		return &frame.FrameContext{
			RetAddrReg: mips64DwarfIPRegNum,
			Regs: map[uint64]frame.DWRule{
				mips64DwarfIPRegNum: frame.DWRule{
					Rule:   frame.RuleOffset,
					Offset: int64(-a.PtrSize()),
				},
				mips64DwarfBPRegNum: frame.DWRule{
					Rule:   frame.RuleOffset,
					Offset: int64(-2 * a.PtrSize()),
				},
				mips64DwarfSPRegNum: frame.DWRule{
	...
	if fctxt.Regs[mips64DwarfBPRegNum].Rule == frame.RuleUndefined {
		fctxt.Regs[mips64DwarfBPRegNum] = frame.DWRule{
			Rule:   frame.RuleFramePointer,
			Reg:    mips64DwarfBPRegNum,
			Offset: 0,
		}
	}
	if fctxt.Regs[mips64DwarfLRRegNum].Rule == frame.RuleUndefined {
		fctxt.Regs[mips64DwarfLRRegNum] = frame.DWRule{
			Rule:   frame.RuleFramePointer,
			Reg:    mips64DwarfLRRegNum,
			Offset: 0,
		}
	}
...
func mips64SwitchStack(it *stackIterator, callFrameRegs *op.DwarfRegisters) bool {
	if it.frame.Current.Fn != nil {
		...
		case "runtime.goexit", "runtime.rt0_go", "runtime.mcall":
			...
			return true
		case "crosscall2":
			//The offsets get from runtime/cgo/asm_mips64.s:10
	...
	case "runtime.asmcgocall":
		...
		off, _ := readIntRaw(it.mem, uintptr(callFrameRegs.SP()+mips64cgocallSPOffsetSaveSlot), int64(it.bi.Arch.PtrSize()))
	...
	case "runtime.cgocallback_gofunc":
		...
		// runtime.cgocallback_gofunc in $GOROOT/src/runtime/asm_mips64.s
		//
...
func mips64RegSize(regnum uint64) int {
	// fp registers
...
var mips64NameToDwarf = func() map[string]int {
	r := make(map[string]int)
	for i := 0; i <= 30; i++ {
		r[fmt.Sprintf("x%d", i)] = i
	}
	r["pc"] = int(mips64DwarfIPRegNum)
	r["lr"] = int(mips64DwarfLRRegNum)
	r["sp"] = 31
	for i := 0; i <= 31; i++ {
...
func mips64RegistersToDwarfRegisters(staticBase uint64, regs Registers) op.DwarfRegisters {
	...
	dregs[mips64DwarfBPRegNum] = op.DwarfRegisterFromUint64(regs.BP())
	if lr, err := regs.Get(int(mips64asm.X30)); err != nil {
		dregs[mips64DwarfLRRegNum] = op.DwarfRegisterFromUint64(lr)
	}

	for dwarfReg, asmReg := range mips64DwarfToHardware {
	...
	dr := op.NewDwarfRegisters(staticBase, dregs, binary.LittleEndian, mips64DwarfIPRegNum, mips64DwarfSPRegNum, mips64DwarfBPRegNum, mips64DwarfLRRegNum)
	dr.SetLoadMoreCallback(loadMoreDwarfRegistersFromSliceFunc(dr, regs, mips64NameToDwarf))
	return *dr
}

func mips64AddrAndStackRegsToDwarfRegisters(staticBase, pc, sp, bp, lr uint64) op.DwarfRegisters {
	dregs := make([]*op.DwarfRegister, mips64DwarfIPRegNum+1)
	dregs[mips64DwarfIPRegNum] = op.DwarfRegisterFromUint64(pc)
	dregs[mips64DwarfSPRegNum] = op.DwarfRegisterFromUint64(sp)
	dregs[mips64DwarfBPRegNum] = op.DwarfRegisterFromUint64(bp)
	dregs[mips64DwarfLRRegNum] = op.DwarfRegisterFromUint64(lr)

	return *op.NewDwarfRegisters(staticBase, dregs, binary.LittleEndian, mips64DwarfIPRegNum, mips64DwarfSPRegNum, mips64DwarfBPRegNum, mips64DwarfLRRegNum)
}

func mips64DwarfRegisterToString(i int, reg *op.DwarfRegister) (name string, floatingPoint bool, repr string) {
	// see mips64DwarfToHardware table for explanation
	switch {
	case i <= 30:
