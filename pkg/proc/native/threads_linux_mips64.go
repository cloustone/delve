...
func (thread *nativeThread) fpRegisters() ([]proc.Register, []byte, error) {
	var err error
	var mips_fpregs linutil.Mips64PtraceFpRegs
	thread.dbp.execPtraceFunc(func() { mips_fpregs.Vregs, err = ptraceGetFpRegset(thread.ID) })
	fpregs := mips_fpregs.Decode()
	if err != nil {
		err = fmt.Errorf("could not get floating point registers: %v", err.Error())
	}
	return fpregs, mips_fpregs.Vregs, err
}

func (t *nativeThread) restoreRegisters(savedRegs proc.Registers) error {
	sr := savedRegs.(*linutil.Mips64Registers)
